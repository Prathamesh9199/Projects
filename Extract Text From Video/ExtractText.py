...
while True:
    ret, frame = cam.read()
    cv2.rectangle(frame, (0, 0), (250, 20), (0, 0, 0), -1)
    cv2.putText(frame, 'X:for Snap;     Q: for quit', (10, 15), 2, 0.5, (255, 255, 255))
...
if key == ord('x'): # if user took snap
    while True:
        cv2.rectangle(frame, (0, 0), (300, 20), (0, 0, 0), -1)
        cv2.putText(frame, 'X:for Extract Text;    R: for Reset', (10, 15), 2, 0.5, (255, 255, 255))
        ...
        # Select Shape
        cv2.setMouseCallback(Window2, shape_selection)

        key = cv2.waitKey(1) & 0xFF

        # Reset
        if len(ref_point) > 2 or key == ord('r'):
            frame = clone.copy()
            ref_point = []
